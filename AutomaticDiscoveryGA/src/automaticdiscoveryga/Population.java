...
public class Population extends ArrayList<Chromossome> {
    ...
    Population(String fn, String token, int cSize, float elitism, Random r, int popSize) throws IOException {
        filename = fn;
        mElitism = elitism;
...
//        fitnessLists.add(this.getPages(filename, token));
//        fitnessLists.addLast(this.getPattern(filename, token, 2));

        PatternList pagesPL = new PatternList(1);
        PatternList duoPL = new PatternList(2);
        PatternList particlesPL = new PatternList(cSize);
...
//        System.out.print("SET oF pAGES : ");
//        for (i = 0; i < setOfPages.length; i++) {
//            System.out.print(setOfPages[i] + " ");
//        }
//        System.out.println();
...
//TODO Do jeito que está implementado está ocorrendo perda de indivíduos com alta aptidão, modificar para que isso não ocorra.
    ...
    void calcFitness() throws FileNotFoundException, IOException {//calcula o fitness da população
     ...
     * @throws IOException
     */
    LinkedList<Chromossome> selectParents() throws IOException {
