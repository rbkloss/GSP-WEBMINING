...
public class Apriori {
    ...
    Apriori(String filename, String token, float supportTreshold, float confidenceTreshold) throws FileNotFoundException, IOException, Exception {
    ...
    class StrongRule {
        ...
        public String toString() {
            StringBuilder answer = new StringBuilder(toString(from) + "->" + toString(to));
...
//        System.out.println(toString(pattern) + " is representend in index as : " + toString(colPos));
        float freq = 0;
    ...
    public ArrayList<ArrayList<Item>> getItemSets(String filename, String token, float treshold) throws FileNotFoundException, IOException, Exception {
        ArrayList<Item> itemSet = getFirstItemSet(filename, token);
        System.out.println("First Item Set was initialized.");
        CompareIntegers comparator = new CompareIntegers();
        Collections.sort(itemSet, comparator);
...
//            System.out.println("item Set :\n" + toString(newItemSet) + "\n\n");
            //Checks each candidate for his support and if it passes the treshold
            //prune
...
//            System.out.println(toString(itemSet));
    ...
    private Item join(Item a, Item b) throws Exception {
        if (a == null || b == null || ((a.getItem().length == 0) || (b.getItem().length == 0))) {
            throw new Exception();
        }
...
//        System.out.println("\t\tk : " + k);
    ...
    private void prune(ArrayList<Item> firstItemSet, ArrayList<Item> itemSet, float treshold) {
        /**
         ...
         */
        float support;
        if (transactionsMatrix == null || itemSet == null || firstItemSet == null) {
        ...
        while (itr.hasNext()) {
            ...
            if (support < treshold) {
//                System.out.println("Removing : " + toString(item) + " frequency was : " + freq + " total of elements : " + this.totalOfTransactions + " support : " + support);
                itr.remove();
            } else {
                item.setFreq(freq);
            }
    ...
    private ArrayList<Boolean[]> initTransactionsMatrix(String filename, String token, ArrayList<Item> firstItemSet) throws FileNotFoundException, IOException {
...
//                answer.get(linePointer)[firstItemSet.indexOf(new Integer[]{element})] = true;
                answer.get(linePointer)[indexOf(firstItemSet, new Integer[]{element})] = true;
            }
            linePointer++;
        }
        answer.trimToSize();
...
//                System.out.printf("the index of object : %s is %d\n", getStringValue(object), i);
    ...
    private ArrayList<Item> getFirstItemSet(String filename, String token) throws FileNotFoundException, IOException {
        FileReader fr = new FileReader(filename);
        BufferedReader br = new BufferedReader(fr);
        int totalElements = 0;
...
//                System.out.println(p[0]);
        ...
        Collections.sort(itemSet, new CompareIntegers());
    ...
    class CompareIntegers implements Comparator<Item> {

        @Override
        public int compare(Item o1, Item o2) {
            if (o1.getItem()[0] < o2.getItem()[0]) {
                return -1;
            } else if (o1.getItem()[0] == o2.getItem()[0]) {
                return 0;
            } else {
                return 1;
            }
...
//        System.out.println("Item is : " + toString(item) + " associations : \n");
...
//                    System.out.println(st1.toString());
    ...
    public float calculateConfidence(StrongRule st, ArrayList<ArrayList<Item>> itemSets) {
        Integer[] X = st.from;
        Integer[] Y = st.to;
...
//        System.out.printf("FreqXY: %f FreqX: %f SupportXY : %f ,Support X : %f \n confidence : Support(XUY)/Support(X) = %f\n", freqXY, freqX, supportXY, supportX, confidence);
    ...
    public static void main(String[] args) throws FileNotFoundException, IOException, Exception {
        ...
        Apriori apriori = new Apriori(filename, token, supportTreshold, confidenceTreshold);
